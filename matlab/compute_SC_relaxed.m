function [K_SC, G_SC, iter_count] = compute_SC(porosity, E0, nu0, tol, max_iter, K_MT, G_MT)
    ...
    relax_factor = 0.2;  % Valore tra 0 e 1 per controllare l'aggiornamento

    % Iteration loop
    for iter = 1:max_iter
        iter_count = iter;  % Update iteration counter

        % Compute Poisson's ratio for the composite
        ...
        % Compute new bulk modulus (SCM update)
        K_new = K0 + (c1 * K_SC * (K1 - K0)) / (K_SC + 3 * gamma_SC * (K1 - K_SC));

        % Compute new shear modulus (SCM update)
        ...
        K_SC = (1 - relax_factor) * K_SC + relax_factor * K_new;
        G_SC = (1 - relax_factor) * G_SC + relax_factor * G_new;
